...
public class CrystalBallPair {
    ...
    /******************* MAPPER *********************/
    public static class CrystalBallPairMapper extends Mapper<LongWritable, Text, Pair, IntWritable>
    {
        ...
        public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException
        {
            ...
            for(int i = 1; i < productIds.length - 1; i++)
            {
                ...
                for(int j = i + 1; ((j < productIds.length) && !rangeCompleted); j++)
                {
                    ...
                    {
                        sum++;
                        addIntoBucket(currentProductId, currentNeighborId, 1);
                    }
                }

                addIntoBucket(currentProductId, null, sum);
            }
        }

        private void addIntoBucket(String currentProductId, String currentNeighborId, int incrementValue) {
            Pair newPair;
            if(currentNeighborId == null)
            {
                newPair = new Pair(currentProductId);
            }
            else
            {
                newPair = new Pair(currentProductId, currentNeighborId);
            }
            if(!bucket.containsKey(newPair))
            {
                bucket.put(newPair, 0);
            }
            bucket.put(newPair,  bucket.get(newPair) + incrementValue);
        }
    ...
    public static class CrystalBallPairPartitioner extends Partitioner<Pair, IntWritable> {
        @Override
        public int getPartition(Pair pair, IntWritable intWritable, int i) {
            if(Integer.parseInt(pair.getFirst()) < 30) return 0;
            if(Integer.parseInt(pair.getFirst()) < 60) return 1;
            return 2;
        }
    }

    /************************ REDUCER ************************/
    public static class CrystalBallPairReducer extends Reducer<Pair, IntWritable, Text, DoubleWritable>
    {
        int numberOfCurrentProduct;
        @Override
        protected void setup(Context context) throws IOException, InterruptedException {
            numberOfCurrentProduct = 0;
            super.setup(context);
        }
        ...
        {
            Iterator<IntWritable> frequencyIterator = frequencies.iterator();
            int totalFrequency = 0;
            while(frequencyIterator.hasNext())
            {
                totalFrequency += frequencyIterator.next().get();
            }

            if(productNeighborPair.isWildCard())
            {
                numberOfCurrentProduct = totalFrequency;
            }
            else
            {
                DoubleWritable average = new DoubleWritable(((double)totalFrequency)/numberOfCurrentProduct);
                context.write(new Text(productNeighborPair.toString()), average);
            }
        }
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        fs.delete(new Path("CrystalBallPair"), true);

        FileInputFormat.addInputPath(job, new Path("input"));
        FileOutputFormat.setOutputPath(job, new Path("CrystalBallPair"));

        job.setMapperClass(CrystalBallPairMapper.class);
        job.setReducerClass(CrystalBallPairReducer.class);

        job.setNumReduceTasks(3);
        job.setPartitionerClass(CrystalBallPairPartitioner.class);
